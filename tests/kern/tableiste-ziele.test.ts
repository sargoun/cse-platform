...
const NOCH_OFFEN: Readonly<Record<string, string>> = {
   ...
   *
   * **Und jetzt sind auch die letzten zwei weg** — `/portal/kunde/auftraege`
   * und `/portal/gruppe/radar`. Beide fielen bis zuletzt auf den Auffang
   * `[...rest]`, also auf genau die Bauzustandsseite, die der Befund dieser
   * Datei ist. Gefunden hat die zweite nicht diese Pruefung, sondern der
   * Mandant im Betrieb: er rief `/portal/gruppe/radar` auf und bekam „Dieses
   * Modul wird noch gebaut" zu sehen.
   *
   * Das ist der Grund, warum diese Liste eine Liste von AUSNAHMEN ist und
   * keine von Zielen: eine Adresse darf hier stehen, solange jemand weiss,
   * dass sie fehlt. Sie darf nicht hier stehen, nachdem sie gebaut ist — und
   * sie darf erst recht nicht hier stehen, wenn niemand mehr hinsieht.
   * Die Liste ist damit leer. Bleibt sie es, hat die dritte Pruefung unten
   * nichts mehr zu tun; fuellt sie sich wieder, steht der Grund daneben.
   */
  /*
