...
| V-116 | **Die GoBD-Löschung eines Dokuments hat ausser dem Test keinen Aufrufer.** `loescheDokument` (`services/dokument/loeschung.ts`) prüft Aufbewahrungsklasse, Löschsperre und Frist — und wird weder von einer Oberfläche noch von einem Aufbewahrungslauf gerufen. `/finanzen/loeschkonzept` sagt zu, dass abgelaufene Dokumente verschwinden; sie verschwinden nicht. | Gefunden von `tests/kern/dienst-verdrahtung.test.ts` | behindert | offen |
