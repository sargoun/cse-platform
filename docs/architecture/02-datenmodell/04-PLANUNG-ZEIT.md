...
| K-06-Fenster | `zeit_intern.arbeitszeit_fenster` | 1 |
| Referenz (global, tenant-free) | `feiertag` | 1 |
...
### 1.1 Database roles, FORCE RLS, and the two sanctioned definer write paths (K-01, K-06, K-08)
...
| `cse_anon` | `EXECUTE` on the three K-08 functions only; **no table in this domain is readable by `cse_anon`** |
| `cse_checkin` | `EXECUTE` on `app.checkin_verbrauchen` only (K-01, K-08) |
...
`FORCE` is applied to **every** table in this domain including `zeit_intern.arbeitszeit_fenster`. The draft exempted the window table, which is the wrong tool: without `FORCE` the owner is exempt and any connection that happens to run as the owner reads every person's cross-entity hours. The window table is protected instead by living in a schema that is not exposed by PostgREST, by having **no policy for `cse_app` at all**, and by holding no column that could identify a shift (§5.12, §6.2).
...
create policy me_definer_insert on medien as permissive for insert to cse_definer
  with check (akteur_art = 'mensch' and erstellt_von_person_id is not null);
...
All database access runs inside `withTenant` / `withGroupScope`, which issue `set_config(..., true)` for the GUCs of K-02 — `app.benutzer_id`, `app.person_id`, `app.mandant_id`, `app.mandant_ids`, `app.scope`, `app.portal`, `app.readonly` (**default `on`**), `app.aal`, `app.sitzung_id`. The active mandant is never read from a URL parameter, a header or a request body (TEN-04, AUT-04, invariant 3). Every accessor of `01-KERN.md` §3.1 fails closed: an unset session produces zero rows in this domain, never all rows.

The `[mandant]` segment under `/portal` (K-07) is routing only, validated against the session; on mismatch the route returns **404, not 403** (AUT-06). The one URL in this domain that carries no session at all is `/check-in/[token]` (K-07, K-08), and it reaches the database exclusively through `app.checkin_verbrauchen` (§9).
...
`dienstplan.arbzg_pruefen` is the right K-06 names for the cross-entity reader; it is granted separately from `dienstplan.arbzg_lesen` because reading a finding in one's own plan and querying another entity's load are different acts (§6.3). Seed grants live in the permission matrix of `01-KERN.md` §14.3; this document requires the six keys above to exist there.
...
Tables carrying it here, enumerated for `src/server/db/rls.ts` (the build fails when an anstellung-hung or person-hung table has no ceiling): `einsatz_zuordnung`, `zeiteintrag`, `zeiteintrag_korrektur` (via `ursprung_zeiteintrag_id`), `checkin_token`, `offline_ereignis`, `medien` (via `zeiteintrag_id`/`erstellt_von_person_id`), `arbeitszeit_verstoss`, `planungs_konflikt`. `einsatz` and `planungsserie` carry no `anstellung_id`; their `mitarbeiter` ceiling is the existence of an own assignment (§5.3). The customer ceiling is the degenerate `app.portal() <> 'kunde'` on every table except `einsatz` and `medien`, where a customer may see the shifts and photos of **their own objects** only, keyed on `kunde_id` resolved through `objekt` — stated per table.

**How EMP-14/EMP-15 are served without widening anything.** A worker with employments in two entities has two `benutzer_mandant` rows (K-14 keeps a manual grant and a derived one side by side). The permissive `t_mandant` policy of a self-readable table gains exactly one additional disjunct, and no other:
...
Three properties make that disjunct safe, and all three are structural rather than promised: the branch matches only rows whose subject **is** the caller (`app.person_id` is a server-set GUC, K-02); these tables carry no monetary column at all (§1.5); and the K-04 ceiling still applies on top, so a `mitarbeiter` session cannot reach another employee's row even if `zeit.lesen` were mis-granted to their role. The disjunct is deliberately not mandant-scoped — that is exactly what "the portal shows shifts across all employments" means (EMP-14) — and it exists on `einsatz`, `einsatz_zuordnung`, `zeiteintrag`, `medien` and nowhere else. It is **never** added to `planungsserie` (a worker sees shifts, not the pattern and its billing anchor), to `planungs_konflikt` or to `arbeitszeit_verstoss` (whether the employer has recorded a breach against a worker is an employer assessment; EMP-13 bounds the portal).

**The one write a worker owns needs exactly one active mandant** (invariant 10). `zeit_einwand` is inserted under `withTenant` for the mandant of the shift being disputed, resolved **server-side** from `zeiteintrag.mandant_id` / `einsatz.mandant_id`; the client submits an entry id, never a mandant. This answers the second half of the review's B11: a worker employed by two entities objects to a security shift while the session's active mandant is `security`, because the route resolved it, not because the browser said so.
...
Two consequences worth stating, because they are what makes §1.4's self-read disjunct defensible: the employee self-read cannot leak a rate, and a cleaning planner reading a shift can never see a security wage. **Any PR that adds a monetary column to a table in this domain is a blocking defect** (§16).
...
| Durations | `integer`, unit in the name (`dauer_netto_minuten`, `zeitabweichung_sek`) | K-16 |
...
with the GoBD implementation `jahresende(belegdatum) + 10 Jahre` and a test for a December and a January Belegdatum, and the MiLoG implementation behind the open question. No purge job may act on a row whose `aufbewahrung_bis` is NULL.

### 1.14 Placeholder marking and configurable operational values (K-17)

Placeholders carry a bold marker (**PLACEHOLDER** / **PROVISIONAL**), a swappable interface and a `// TODO(client): <exact question>`; §17 collects them for `DECISIONS.md` under **Open**, and `pnpm lint:todo` fails when a `// TODO(client)` in this domain has no matching row there. Operational values that are neither legal facts nor tariff values live in `mandant_einstellung(schluessel, wert jsonb)` read through `app.einstellung(schluessel)` (the mechanism `03-GEWERKE.md` §1.16 requires of `01-KERN.md`), each with its default stated in §17.2.
...
Storing a fact and evaluating it as a performance measure are different acts, and only the second is gated — which is why invariant 5's `zeitabweichung_sek` survives an unanswered O-06 while the ranking screen does not.
...
| `anstellung` | KERN | `unique (mandant_id, id)`, **`unique (id, person_id)`** | `id`, `person_id`, `mandant_id`, `status`, `eintritt`, `austritt`, `archiviert_am` | D-09 |
...
| `auftrag_leistung` | CRM-OPS | `unique (mandant_id, id)`, **`unique (mandant_id, auftrag_id, id)`** | `id`, `auftrag_id`, `abrechnungsart`, `gueltig_ab`, `gueltig_bis` | FIN-01, FIN-07 |
...
1. **`01-KERN.md` §3.5 — the `cse_definer` read registry** gains `einsatz` and `einsatz_zuordnung` (already required by `03-GEWERKE.md` §2.3 item 1), and the registry's stated exception "no `INSERT`/`UPDATE`/`DELETE` policy on a tenant table except the ones K-06 sanctions" is widened to "except the ones **K-06 and K-08** sanction", enumerated in §1.1 of this document. Without that, the check-in path of K-08 cannot insert the `zeiteintrag` K-08 says it inserts.
...
5. **`01-KERN.md` §14.3 — the seeded permission matrix** must contain the eleven right keys of §1.3, including `dienstplan.arbzg_pruefen` (K-06) and `gruppe.dienstplan.arbzg_lesen`.
...
-- TODO(client): Welche Zustände braucht eine Einsatzzuordnung zwischen Planung und Ausführung
...
-- TODO(client): Gibt es vor Stundenkonto und Abrechnung eine fachliche Freigabe der Zeiten, und wer
--               erteilt sie — Objektleitung, Planung oder Buchhaltung?
...
-- TODO(client): Welche Korrekturgründe soll die Auswertung unterscheiden?
...
`pause_minuten` is a total per entry, and pause **events** are not modelled. The draft carried `pause_start`/`pause_ende` in the offline vocabulary while the schema had nowhere to put them, which would have produced submissions the processor could only reject. `offline_ereignis_art` therefore has one `pause` value carrying a minute total in the payload. `// TODO(client): Werden Pausen gestempelt (Start/Ende) oder als Minutensumme je Schicht erfasst? Gestempelte Pausen brauchen eine eigene Tabelle und ändern die ArbZG-Auswertung.` Until answered, the ArbZG detector reads the recorded total and reports a missing break as a finding; it never invents one.
...
The single cross-tenant edge in the whole diagram is `person → zeit_intern.arbeitszeit_fenster`. Everything else stops at a `mandant_id`, and the only relation that crosses it is reachable through one audited `SECURITY DEFINER` function (§6).
...
| `feiertage_ueberspringen` | boolean | **no default** | — | §8.5. `turnus` supplies it from `feiertagsregel` (CLN-03); for `posten`/`veranstaltung` the creating service reads `app.einstellung('zeit.feiertage_ueberspringen_posten')`, whose default is `false` |
...
- **RLS:** standard, module `dienstplan`, plus the EMP-14 self-read disjunct of §1.4 expressed against the assignment, because `einsatz` carries no `person_id`:
  ...
  Writes stay on `mandant_id = app.aktiver_mandant()` with `dienstplan.schreiben`. The customer ceiling permits `select` where the shift's `objekt.kunde_id = app.aktueller_kunde()` and only the columns a customer may see; the `notiz`, `anforderung_snapshot` and `quell_schluessel` columns are excluded by the column grant of `src/server/db/rls.ts`.
...
| `funktion` | text | yes | — | role on the shift; deliberately **not** an enum. `// TODO(client): Welche Funktionen gibt es auf einer Schicht (Objektleiter, Vorarbeiter, Springer, Sicherheitsmitarbeiter)?` |
...
- **RLS:** standard, module `dienstplan`, plus `or person_id = app.aktuelle_person()` on the read branch (EMP-14), plus the K-04 ceiling. A worker sees their own assignment rows in every entity they work for; never anyone else's, and never a rate — there is none here.
- **Constraints/triggers:**
  ...
  - `ez_qualifikation_gate` (`BEFORE INSERT OR UPDATE`): the last-line SEC-04/LEG-04 enforcement of §11 — refuses the row and writes `qualifikation_snapshot` in the same statement.
...
| `nacherfasst` | boolean | no | `false` | TIM-09 "flagged as late"; `check (not nacherfasst or behauptet_beginn is not null)` |
...
| `gesperrt_am` | timestamptz | yes | — | set when the entry's `stundenkonto` month locks (EMP-04); after this a correction books into the next open month (§12.2), and the entry itself is never re-dated |
...
- **RLS:** standard, module `zeit`, plus `or person_id = app.aktuelle_person()` on the **read** branch only (EMP-14/15), plus the K-04 ceiling. **No employee `INSERT` or `UPDATE` branch of any kind** — EMP-07 is categorical: the worker never writes a time entry, not even their own; they write a `zeit_einwand` (KERN §6.27). No `DELETE` policy (invariant 8).
- **Constraints/triggers:**
...
- **RLS:** standard, module `zeit`, plus `or erstellt_von_person_id = app.aktuelle_person()` on the read branch (a worker sees the photos they took), plus the K-04 ceiling via `zeiteintrag_id`. Signed-URL issuance is a server route that re-checks the same predicate — a Storage policy alone would not (SEC-A6, DOC-04).
- **Constraints/triggers:**
  - `me_bezug_pruefen` (`BEFORE INSERT OR UPDATE`, `SECURITY DEFINER`): looks the parent up through the registry and raises unless it exists **and** carries the same `mandant_id`. This is what a foreign key would have given, obtained by other means (§5.8.1, §15.4).
...
The server-side landing zone for events a phone captured with no network: what was claimed, when it actually arrived, and what a human decided about it (TIM-09).

| Column | Type | Null | Default | Notes |
|---|---|---|---|---|
| `id` | uuid | no | `gen_random_uuid()` | PK |
| `mandant_id` | uuid | **yes** | — | resolved server-side from the token. **Documented exception to K-16**: `check (mandant_id is not null or status = 'abgelehnt')`. When the presented token does not resolve there is no tenant to resolve *to*, and the submissions that cannot be stored would be precisely the disputed ones — a worker claiming they stamped in with a link that had been revoked (review B13, LEG-02) |
...
| `checkin_token_id` · `einsatz_id` · `einsatz_zuordnung_id` · `anstellung_id` · `person_id` | uuid | yes | — | all nullable, because a malformed or expired submission is recorded first and rejected second; composite FKs where the tenant is known |
...
- **Indexes:** `oe_idem_uk unique (geraet_id, client_ereignis_id)` — **not** prefixed with `mandant_id`, so an unresolved submission still deduplicates (review B13); `oe_queue_idx on (mandant_id, status, empfangen_am) where status in ('empfangen','zugeordnet','manuelle_pruefung')` — the processor's and the planner's queues; `oe_unzugeordnet_idx on (empfangen_am) where mandant_id is null`; `oe_person_idx on (person_id, behauptete_zeit) where person_id is not null` — reconstructing what one worker submitted late; `oe_geraet_idx on (geraet_id, empfangen_am)`.
- **RLS:** standard for read, module `zeit`, right `zeit.nacherfassung_pruefen`, plus the K-04 ceiling. A row with `mandant_id is null` matches no policy and is therefore invisible to every `cse_app` session — fail-closed by construction; it is reachable only through `app.offline_unzugeordnet_lesen()` (`SECURITY DEFINER`, requires `system.betrieb_lesen`, writes `audit_log`). Ingestion is `app.offline_ereignis_annehmen()` under `cse_checkin` (§9.4); there is no `INSERT` policy for `cse_app` and none for `cse_anon`.
...
| `blockiert` | boolean | no | `false` | whether this finding blocked the planning action. **PLACEHOLDER** — SPEC TIM-06 says "warnings", not "blocks", and SEC-04 says "hard block" for certificates only. Fed from `app.einstellung('zeit.konflikt_blockiert')` per `art`; ships `false` except `qualifikation_entfallen`, where SEC-04 makes it `true`. `// TODO(client): Welche Konflikte sollen das Speichern verhindern und welche nur warnen?` |
| `details` | jsonb | no | `'{}'::jsonb` | the contributing intervals, reduced to durations and boundaries for a foreign entity |
| `fingerprint` | text | no | — | `sha256(mandant_id ‖ person_id ‖ art ‖ berlin_tag)` — see §6.7 for why the day and not the instants |
...
|---|---|---|---|---|
| `id` | uuid | no | `gen_random_uuid()` | PK |
| `zuordnung_quelle_id` | uuid | no | — | **the `einsatz_zuordnung` this window derives from** — carried by the `plan` row *and* by the `ist` row (K-06). FK added in Phase 5 per `01-KERN.md` §19 |
...
begin
  ...
  perform app.protokolliere('arbzg.aggregat_gelesen', 'person', p_person,
            jsonb_build_object('von', p_von, 'bis', p_bis, 'zeilen', v_fremd));
end $$;
...
```sql
create function zeit.arbzg_belastung_job(p_person uuid, p_von timestamptz, p_bis timestamptz)
...
revoke all on function zeit.arbzg_belastung_job(uuid, timestamptz, timestamptz) from public;
grant execute on function zeit.arbzg_belastung_job(uuid, timestamptz, timestamptz) to cse_job;
```

Four properties make it acceptable: it lives in `zeit_intern`'s function namespace and is executable by `cse_job` only; it writes `audit_log` on every invocation; the **route-manifest test of K-08 asserts no HTTP route reaches it**, directly or through a service module reachable from a route; and the job iterates persons from `anstellung` rather than from a tenant session, so it never depends on a GUC that a cron run cannot set. It returns `mandant_id` because the writer of §6.6 needs to know which entities to mirror a finding into — that value never leaves the job process, and the rows it writes carry only what §5.11 permits.
...
begin
  ...
  elsif current_user <> 'cse_job' then
    raise exception 'nicht berechtigt' using errcode = '42501';
  end if;
...
`zeit.ursache_fuer_mandant` is the reduction K-06 requires: for a window that did not originate in the target mandant it emits `{eigen:false, fenster_gruppe, beginn, ende, minuten}` and drops every identifier including `mandant_id`. **Only caller:** `src/server/services/arbzg/`.
...
// TODO(client): Wie werden die beiden Nächte der Zeitumstellung bezahlt — zählt die tatsächlich
...
| Monthly PDF (EMP-06) | `freigegeben_am is not null` | EMP-06 |
...
```sql
  if t.zweck = 'checkin' then
    ...
    select t.mandant_id, t.anstellung_id, t.person_id, t.einsatz_id, t.einsatz_zuordnung_id,
           now(), 'server_uhr', 'checkin_token',
           p_geraete_zeit, t.id, 'laufend',
           'mensch', b.id, t.person_id, …
      from public.benutzer b where b.person_id = t.person_id
...
`beginn_zeitpunkt` is `now()` — the **server clock**, never anything the phone sent (invariant 5, TIM-08). The device reading goes into `geraete_zeit_beginn`, and `zeit.stempel_feldzeit()` derives `zeitabweichung_beginn_sek` from it. The actor is `mensch` with the worker's own `benutzer` resolved from `person_id` (§1.6), not an anonymous token identity. Finally `checkin_token.eingeloest_zeiteintrag_id` is set, and `app.protokolliere` records the redemption with the IP (SEC-A9).

**Concurrency test:** N simultaneous requests against one token yield exactly one `zeiteintrag` (K-09).
...
The device queues events locally and replays them to `app.offline_ereignis_annehmen(...)` under `cse_checkin`. The honest reading of TIM-08 **and** invariant 5 together:
...
// TODO(client): Welche interne Frist gilt für die Nacherfassung — die gesetzlichen sieben
...
The requirement set is resolved by `app.qualifikationsanforderung(p_einsatz)` and evaluated by `app.einsatz_qualifikation_erfuellt(p_anstellung, p_einsatz)` — both owned by `03-GEWERKE.md` §1.10, both reading `nachweis` **and `bewacher_eintrag`** at the person head. This document supplies the two proof columns and the trigger:

- `ez_qualifikation_gate` (`BEFORE INSERT OR UPDATE ON einsatz_zuordnung`) calls the function with the shift's `beginn_zeitpunkt` as the reference date, writes the result into `qualifikation_snapshot` and `qualifikation_geprueft_am`, and **raises** when a `zwingend` requirement is unmet. SEC-04 demands enforcement in the service layer, and `src/server/services/dienstplan/zuordnen.ts` is the primary gate; the trigger exists so an import, a console statement or a future service that forgets cannot bypass it.
...
`/portal/mein/…` (K-07) runs under `withTenant` for the person's primary employment or under `withGroupScope` for the combined view; in both cases `app.portal() = 'mitarbeiter'`, so the K-04 ceiling is active and `app.readonly` is `on` for every aggregate screen. The reads are:

| Screen | Path | Index |
|---|---|---|
| My shifts, next 14 days, all employments (EMP-02, EMP-14) | `einsatz_zuordnung` join `einsatz`, self-read disjunct | `ez_person_zeit_idx` |
...
| Objection (EMP-07) | `zeit_einwand` insert, mandant resolved server-side (§1.4) | KERN |
...
`SECURITY DEFINER`, keyed on `app.aktuelle_person()`, which the feed route sets after resolving the token to a `benutzer` inside its own `withTenant`/`withGroupScope` transaction — the route is not a fourth K-08 function, because it opens a session of its own (`01-KERN.md` §6.10). The feed carries **no** customer commercials, no rate, no `notiz`, no conflict and no colleague (EMP-13): a shift, a place, a time, an entity label. Revoking the token in the portal invalidates the URL immediately, and every fetch stamps `letzte_nutzung_am` so an abandoned feed is visible.

---

## 13. Retention, Löschsperre and erasure (LEG-01, LEG-02, LEG-09, DOC-07)

Every table carries `aufbewahrung_bis date` and `loeschsperre boolean not null default true` (§1.13), and each is mapped to a class in `dokument_aufbewahrung`:
...
| `einsatz`, `einsatz_zuordnung`, `planungsserie` | `dienstplan` | none stated | `// TODO(client)` — planning data is evidence in an ArbZG inspection and in a wage dispute; the period is O-25 |
| `medien` | `einsatz_medien` | none stated | DOC-07 requires a rule per category; O-25 |
| `checkin_token`, `offline_ereignis` | `checkin_token` / `offline_ereignis` | none stated | shortest-lived personal data in the domain (IP, user agent, device id); the rejected claims are wage-dispute evidence, so the period is asked, not guessed |
| `planungs_konflikt`, `arbeitszeit_verstoss` | `arbzg_befund` | none stated | `01-KERN.md` §16(h) asks the same question for audit classes |

Rules that hold regardless of the answers: `job:aufbewahrung` writes `aufbewahrung_bis` and never deletes; the purge path may consider only rows with `aufbewahrung_bis is not null and aufbewahrung_bis < app.berlin_heute() and not loeschsperre`; and even then it does not `DELETE` — every table here carries `kern.verhindere_loeschung()` (invariant 8). What "purge" means for a MiLoG record whose retention has expired while the DSGVO deletion concept wants it gone is a determination for the client's DPO, and the schema's job is to record the deadline and the lock, which it does.

**DSGVO erasure (LEG-09)** of a person whose costed records must survive is `person.anonymisiert_am` in `01-KERN.md` §15; this domain contributes: `medien.storage_geloescht_am` (binary removed, row kept as a tombstone), `offline_ereignis.nutzlast_roh` overwritten with a hash-only marker, `checkin_token.ip_adresse`/`user_agent` nulled, and `zeiteintrag` untouched — the hours themselves are the record the law requires to exist.
...
| `zeit.stempel_feldzeit()` | `zeiteintrag`, `offline_ereignis`, `BEFORE INSERT OR UPDATE` | server instant + `zeitabweichung_sek` | TIM-08, TIM-09 |
...
| `ez_qualifikation_gate` | `einsatz_zuordnung`, `BEFORE INSERT OR UPDATE` | SEC-04 hard block + snapshot | SEC-04, LEG-04 |
...
| `job:arbzg_detektor` | nightly | `cse_job`; `execute` on `zeit.arbzg_belastung_job`, `app.arbzg_befund_schreiben` | `arbeitszeit_verstoss`, `planungs_konflikt` | TIM-14, LEG-03 |
| `job:arbzg_fenster_abgleich` | nightly | `cse_job`; `select/update` on `zeit_intern.arbeitszeit_fenster` | reports drift as `kritisch`; never silently repairs | TIM-14 |
...
| `job:offline_altbestand` | hourly | `cse_job` | moves stale claims to `manuelle_pruefung` | TIM-09 |
...
| `checkin_token` | `(mandant_id, einsatz_zuordnung_id) → einsatz_zuordnung (mandant_id, id)` | §5.4 |
| `zeiteintrag` | `(mandant_id, einsatz_zuordnung_id)`, `(mandant_id, anstellung_id)`, `(anstellung_id, person_id)`, `(mandant_id, auftrag_leistung_id)`, `(mandant_id, objekt_id)`, `(mandant_id, revier_id)`, `(mandant_id, posten_id)`, `(mandant_id, projekt_id)` | as above |
...
| `planungs_konflikt` | `(mandant_id, arbeitszeit_verstoss_id)`, `(mandant_id, einsatz_id)`, `(mandant_id, einsatz_zuordnung_id)`, `(mandant_id, zeiteintrag_id)` | §5.3–§5.11 |
| `medien` | `bezug_tabelle → medien_bezug (tabelle)` plus the tenant assertion trigger | §5.8.1 |
...
2. An RLS policy without a `hat_recht` conjunct, or a self-read disjunct that is not additionally bounded by the K-04 ceiling (K-03, K-04, review B1).
...
7. A `SELECT` policy, a `GRANT`, a view, or a `cse_app`-reachable function over `zeit_intern.arbeitszeit_fenster` (§5.12, K-06).
...
13. Any write path that runs without exactly one active mandant, or any group-scope write policy (invariant 10, TEN-04, TEN-05).
...
| # | Question | Blocks |
|---|---|---|
| O-30 | Ab wann läuft die zweijährige Aufbewahrung nach §17 Abs. 1 MiLoG — ab dem Tag der Arbeitsleistung, ab Erstellung der Aufzeichnung oder ab Monats- bzw. Jahresende? Und gilt für einen bereits abgerechneten Zeiteintrag zusätzlich die zehnjährige GoBD-Frist ab Ende des Kalenderjahres (§147 Abs. 3 AO)? | `aufbewahrung_bis`, `job:aufbewahrung`, LEG-01, LEG-02 |
| O-31 | Wie werden die beiden Nächte der Zeitumstellung bezahlt — nach tatsächlich geleisteter Zeit (7 h bzw. 9 h) oder nach geplanter Schichtlänge? Bei doppelt vorhandener Ortszeit in der Rückstellungsnacht: früherer oder späterer Zeitpunkt? | `DstStrategie`, TIM-06, EMP-04 |
| O-32 | Wie erreicht der Check-in-Link den Mitarbeitenden — SMS, E-Mail, aushängender QR-Code am Objekt oder Portal-Link — wer ist der SMS-Anbieter (EU-Verarbeitung, AVV), und wer trägt die Kosten? | `checkin_token.ausgabe_kanal`, TIM-07, EMP-01 |
| O-33 | Wie lange nach Schichtende bleibt der Check-out-Link gültig, und was geschieht, wenn deutlich länger gearbeitet wird als geplant? | `zeit.checkout_toleranz_minuten`, TIM-07 |
| O-34 | Welche interne Frist gilt für die Nacherfassung, unterhalb der gesetzlichen Höchstfrist von sieben Kalendertagen (§17 Abs. 1 MiLoG), und wer wird beim Überschreiten informiert? | `AufzeichnungsfristRegel`, TIM-09, TIM-13 |
| O-35 | Welche Konflikte sollen das Speichern verhindern und welche nur warnen (Überschneidung, ArbZG, Qualifikation)? SPEC nennt „Warnungen" für ArbZG und einen „harten Block" nur für Qualifikationen. | `planungs_konflikt.blockiert`, TIM-05, TIM-06 |
| O-36 | Werden Schichtposten und Veranstaltungsdienste an gesetzlichen Feiertagen regulär besetzt, und arbeitet die Gruppe an Objekten außerhalb Berlins (welche Bundesländer)? | `feiertage_ueberspringen`, `feiertag_bundesland`, CLN-03 |
| O-37 | Werden Pausen gestempelt (Start/Ende) oder als Minutensumme je Schicht erfasst? | `pause_minuten`, §3.4, TIM-06 |
| O-38 | Gibt es Zeiten ohne Auftragsbezug — interne Arbeit, Schulung, Bereitschaft, Fahrzeit — und wie werden sie kostenmäßig behandelt? | `zeiteintrag.auftrag_leistung_id`, FIN-18 |
| O-39 | Gibt es vor Stundenkonto und Abrechnung eine fachliche Freigabe der Zeiten, und wer erteilt sie? | `ZeitFreigabe`, EMP-04, FIN-07 |
| O-40 | Welche Funktionen gibt es auf einer Schicht (Objektleiter, Vorarbeiter, Springer), und welche Zustände braucht eine Einsatzzuordnung? | `funktion`, `zuordnung_status`, TIM-04 |
| O-41 | Soll die Plattform Sonn- und Feiertagsarbeit nach §§9–13 ArbZG nachweisen (Ausnahmetatbestand je Bereich, Ersatzruhetag)? | §6.7, LEG-03 |
| O-42 | **O-06 erweitern:** Betrifft die Mitbestimmung nach §87 Abs. 1 Nr. 6 BetrVG neben der Geolokalisierung auch Geräteabweichung, Gerätekennung, Nicht-erschienen-Auswertung und Korrekturstatistiken? Alle sind Einrichtungen zur Verhaltens- und Leistungskontrolle. | §1.15, LEG-10, LEG-09 |
...
**O-06** (Betriebsrat → LEG-10, widened by O-42), **O-09** (data volumes → the index decision of §7.4), **O-18** (ArbZG 10 h exception and compensation window → §6.7), **O-25** (retention per category → §13), **O-27** (payroll export target → the ACC-12 consumer of §7.3), and `01-KERN.md` §16 items (i) Bewacherregister status vocabulary → §11.1, (k) AUT-07 thresholds → §9.2, (h) audit retention → §13.
...
**Cross-entity ArbZG (TIM-14, D-09, K-06)** — the seven cases of §6.8, in full, including the concurrency case and the zero-rows-on-direct-select case.
...
10. A write attempted under `app.scope = 'gruppe'` is refused by Postgres with the service guard disabled (invariant 10).
...
16. An unresolvable token still produces a row in `offline_ereignis` with `mandant_id is null`, and that row is invisible to every `cse_app` session (review B13).
...
19. Lock March, correct a March entry in May: March's `stundenkonto` totals are unchanged and the delta appears in the first open month with `korrektur_fuer_stundenkonto_id` set (EMP-04, §12.2).
...
26. A certificate expiring between assignment and shift produces a `planungs_konflikt` with `art = 'qualifikation_entfallen'` on the next nightly run (review B8).
...
30. `job:aufbewahrung` never sets `aufbewahrung_bis` on a row under `loeschsperre`, and the purge path selects nothing whose `aufbewahrung_bis` is NULL.
...
| Column | Target | Phase |
|---|---|---|
| `zeit_intern.arbeitszeit_fenster.zuordnung_quelle_id` | `einsatz_zuordnung` | Phase 5 (K-06, `01-KERN.md` §19) |
