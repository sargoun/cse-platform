...
Two of these rows are corrections, and both were the kind of defect K-20 was written for. **`app.aktuelle_kunden()` resolves from the session's `kunde_zugang` binding** (`02-CRM-OPERATIONS.md` §4.1 owns that table), never through `app.aktiver_mandant()`: in kunde scope that binding *is* the subject of the request. The scalar `app.aktueller_kunde()` is `(app.aktuelle_kunden())[1]`, so in that scope it returns **one binding of several, arbitrarily** — not NULL — and a customer-facing screen written against it shows one entity's shifts and silently omits the rest (CRM-06); under the draft's `aktiver_mandant()`-derived definition it would have returned NULL and emptied the screen outright. Both readings are wrong and neither raises an error. This document therefore uses the **array** form in every `t_kunde` policy and every customer ceiling below; the scalar survives only as a mandant-scope convenience, defined as `(app.aktuelle_kunden())[1]`. And **`app.portal()` is bound when the scope is entered** and is never recomputed from `aktiver_mandant` (K-04, K-20): recomputing it would fall through to the fail-closed `'mitarbeiter'` in group, person and kunde scope, which fires every `p_ma_ceiling` of §1.4 inside the group view — a Leitung reading nothing — and ceilings a customer as though they were an employee.
...
**The group-read key for ArbZG findings is `gruppe.dienstplan.arbzg_lesen`, and it is expressible.** An earlier pass of this document argued the opposite — that the group grammar was `gruppe.<modul>.<aktion>` with a bare enum `aktion`, so `arbzg_lesen` could never be inserted and the key had to collapse into `gruppe.dienstplan.lesen`. The catalogue owner settled it the other way: `03-AUTH-BERECHTIGUNGEN.md` §7.2 widened the group grammar to `gruppe.<modul>.[<objekt> '_'] <aktion>` **precisely so this key and `gruppe.system.audit_lesen` can exist** — parsed as `objekt = dienstplan_arbzg`, `aktion = lesen`, which keeps the three-segment `CHECK` of `01-KERN.md` §6.6 intact and both columns enum-valid — lists it in §12.1, and names this section as the document that needs it. The catalogue owns the grammar and the key, so this document follows it.

Keeping the collapsed spelling would not have been neutral. An ArbZG finding is the one row in this domain that is *about a person across entities* (K-06, D-09), and `gruppe.dienstplan.lesen` is the ordinary per-module group read: binding the two together grants everyone who may see a group roster the cross-entity working-time findings of every employee in the group, which is the aggregation §6.3 confines to a `SECURITY DEFINER` function returning durations and boundaries and nothing else. A separate key is what keeps the two grants separable.

`dienstplan.arbzg_pruefen` is the right K-06 names for the cross-entity reader; it is granted separately from `dienstplan.arbzg_lesen` because reading a finding in one's own plan and querying another entity's load are different acts (§6.3). Seed grants live in the permission matrix of `01-KERN.md` §14.3 against the catalogue of `03-AUTH-BERECHTIGUNGEN.md`, and this document requires **exactly these sixteen keys** to exist there — listed, not counted, because a count is what the permission-matrix seed cannot be written against:

`dienstplan.lesen` · `dienstplan.schreiben` · `dienstplan.veroeffentlichen` · `dienstplan.konflikt_quittieren` · `dienstplan.arbzg_lesen` · `dienstplan.arbzg_pruefen` · `dienstplan.arbzg_uebersteuern` · `zeit.lesen` · `zeit.schreiben` · `zeit.korrigieren` · `zeit.checkin_verwalten` · `zeit.nacherfassung_pruefen` · `zeit.abrechnung_freigeben` · `system.betrieb_lesen` · `gruppe.dienstplan.lesen` · `gruppe.dienstplan.arbzg_lesen` · `gruppe.zeit.lesen`
...
(§2.3 item 5 repeats the same list verbatim, and a seed test compares the two.) `zeit.abrechnung_freigeben` **joined the list with D-611** and was deliberately off it until then: whether a release step between worked time and billing exists at all was **O-39**, and seeding a right for a step the client may not want would have made the placeholder load-bearing (§3.3, K-17). The client answered yes — a human releases weekly, before invoicing — so `drizzle/0371` binds the key to `super_admin` and `admin`, and `03-AUTH-BERECHTIGUNGEN.md` §12.4 carries the same binding. `leitung` is grantable per Gesellschaft rather than bound by default (D-612): releasing to billing is a commercial act, not a shift act. **O-861 is still open** — the unit of release and whether it can be retracted — and it is a question about this step, not about whether the step exists.
...
**All five are `mandant_einstellung` keys, not columns of `mandant`** (K-21) — five of the seven `01-KERN.md` §6.30 enumerates and seeds, the other two being `03-GEWERKE.md` §1.16's `geo.erfassung_erlaubt` and `wachbuch.uebergabe_fenster`. There is no `mandant.geo_erfassung_aktiv` column and no `mandant.ueberwachung_aktiv` column: `01-KERN.md` §6.1 declares neither, and a boolean on `mandant` could not express the per-feature granularity §87 Abs. 1 Nr. 6 BetrVG makes necessary — one switch for five distinct *Einrichtungen* is the very collapse this section exists to undo. `05-API-KARTE.md` §C/§D and `08-PR-PLAN.md` PR 4 state the same. **`01-KERN.md` §6.30 owns the keys; this table owns what each one does while it is off**, and `z_geo_gate` (§5.6) reads the first of them through `app.einstellung`:
...
4. **`01-KERN.md`** owns `mandant_einstellung(id, mandant_id, schluessel, wert jsonb)` with `unique (mandant_id, schluessel)` and `app.einstellung(p_schluessel)` — K-21 fixes both the owner and the shape, and `03-GEWERKE.md` §1.16 places the same requirement. This document reads six settings (§17.2) and **declares none of them locally**. **Met:** the five O-06 monitoring switches of §1.15 are enumerated and seeded by `01-KERN.md` §6.30/§14 as `mandant_einstellung` keys, and `05-API-KARTE.md` §C/§D and `08-PR-PLAN.md` PR 4 now state in the same words that there is no `mandant.geo_erfassung_aktiv` and no `mandant.ueberwachung_aktiv` column. Nothing outstanding.
5. **`03-AUTH-BERECHTIGUNGEN.md` §7.4/§12 — the permission catalogue (K-19)** must carry a row for each of these seventeen right keys, and **`01-KERN.md` §14.3 — the seeded permission matrix** must seed them, listed rather than counted so the seed can be written against them: `dienstplan.lesen`, `dienstplan.schreiben`, `dienstplan.veroeffentlichen`, `dienstplan.konflikt_quittieren`, `dienstplan.arbzg_lesen`, `dienstplan.arbzg_pruefen` (K-06), `dienstplan.arbzg_uebersteuern`, `zeit.lesen`, `zeit.schreiben`, `zeit.korrigieren`, `zeit.checkin_verwalten`, `zeit.nacherfassung_pruefen`, `zeit.abrechnung_freigeben` (D-611), `system.betrieb_lesen` (§5.13), `gruppe.dienstplan.lesen`, `gruppe.dienstplan.arbzg_lesen` (§1.3 — expressible under §7.2's widened group grammar, `objekt = dienstplan_arbzg`, `aktion = lesen`), `gruppe.zeit.lesen`. Module `dienstplan` must exist in §7.4's closed module list — `01-KERN.md` §6.6's assertion that "there is no module `dienstplan`" and its seeding of `dienstplan.arbzg_pruefen` under a module `einsatz` are both wrong, and K-06 fixes the key *string*, not the module column. `berechtigung_aktion` must contain `schreiben`, `pruefen`, `veroeffentlichen`, `quittieren`, `korrigieren`, `uebersteuern` and `verwalten`, or the keys above cannot be inserted and every policy naming them is permanently false (K-19). `zeit.abrechnung_freigeben` was not seeded until O-39 was answered; D-611 answered it and `drizzle/0371` binds it to `super_admin` and `admin`, `leitung` grantable per Gesellschaft (D-612, §1.3).
...
14. **`00-KONVENTIONEN.md` K-08, `01-KERN.md` §3.5, `03-AUTH-BERECHTIGUNGEN.md`, `05-API-KARTE.md` §B and `07-INTEGRATIONEN.md` §6.2 — the register row for `app.checkin_verbrauchen` carries the owner's full signature. Met: all five now do.** An earlier pass had all five writing the three-argument `app.checkin_verbrauchen(token_hash, geraet_zeit, ip)`; the body is declared here (§9.1) and takes five: `(p_token_hash text, p_geraete_zeit timestamptz, p_ip inet, p_user_agent text, p_geo jsonb default null)`. The two extra parameters are not optional decoration — `checkin_token.user_agent` (§5.5) and the LEG-10 geo capture (§9.5) have no other source, and dropping them would leave two declared columns with no writer. **Postgres overloads on the argument list**, so `GRANT EXECUTE … TO cse_checkin` written against the three-argument signature does not reach the five-argument function: the grant succeeds against nothing, and the check-in endpoint fails closed at runtime with a "function does not exist" that no schema test catches. One signature, in all six documents.
...
-- O-39 ist beantwortet (D-611): ja, ein Mensch gibt woechentlich frei, vor der Fakturierung, und
-- es erteilt der zustaendige Admin — `zeit.abrechnung_freigeben`, gebunden in `drizzle/0371`.
-- // TODO(client, O-861): In welcher Einheit wird freigegeben — je Eintrag, je Woche, je Person, je Monat —, und laesst sich eine erteilte Freigabe zuruecknehmen, solange nichts abgerechnet ist?
...
- **RLS:** read under the standard policy, module `dienstplan`, right `dienstplan.arbzg_lesen`, plus the K-04 ceiling; group read under **`gruppe.dienstplan.arbzg_lesen`** — its own group key, separate from the roster's `gruppe.dienstplan.lesen`, and expressible under §7.2's widened group grammar as `objekt = dienstplan_arbzg`, `aktion = lesen` (§1.3). The separation is the point: seeing a group roster and seeing a person's cross-entity working-time findings are different grants (K-06). **There is no `INSERT` or `UPDATE` policy for `cse_app` at all** (K-06): a breach spanning two entities must be recorded in both, and a request scoped to mandant A cannot write a row in mandant B. Findings are written only by `app.arbzg_befund_schreiben(...)` (§6.6). The one field a planner changes — the acknowledgement — goes through `app.arbzg_befund_quittieren(p_id, p_begruendung)`, `SECURITY DEFINER`, which re-checks `dienstplan.arbzg_lesen` in the finding's own mandant and writes `audit_log`.
...
21. A policy or ceiling keyed on the **scalar** `app.aktueller_kunde()` anywhere reachable from `kunde` scope — it is `(app.aktuelle_kunden())[1]` and returns one of the customer's bindings arbitrarily, so the test greps `pg_policies` for the name rather than asserting a return value (K-20, §1.2) — or on any accessor that resolves through `app.aktiver_mandant()` in a scope where `app.mandant_id` is NULL. The array form `app.aktuelle_kunden()` is the only customer key in this domain.
...
| ~~O-93~~ **beantwortet — D-618** | `zeit-checkin-kanal` | Wie erreicht der Check-in-Link den Mitarbeitenden? **Für eine ANGEMELDETE Arbeiterin gar nicht.** Der Token-Link bleibt unverändert für jeden ohne Sitzung (QR am Objekt, Link der Planung, geteiltes Gerät); wer im Arbeiterportal angemeldet ist, stempelt über die Sitzung — sie ist der stärkere Beweis, weil ein Token ein Inhaberpapier ist. Ersetzt wird die ZUSTELLUNG der Marke, nicht die Marke: `ausgabe_kanal` trägt dann `portal`. Die SMS-Kostenfrage entfällt für den täglichen Weg; O-82 bleibt offen und blockiert nichts mehr | D-618, `checkin_token.ausgabe_kanal`, TIM-07, EMP-01, EMP-07, K-08 |
...
| ~~O-39~~ **beantwortet — D-611** | `zeit-freigabeschritt` | Gibt es vor Stundenkonto und Abrechnung eine fachliche Freigabe der Zeiten, und wer erteilt sie? **Ja, woechentlich, vor der Fakturierung; der zustaendige Admin.** Gebunden in `drizzle/0371` (`leitung` je Gesellschaft anlegbar — D-612) | `ZeitFreigabe`, EMP-04, FIN-07 |
...
33. `zeit.abrechnung_freigeben` appeared in **no** policy, no seed and no route gate of this domain while O-39 was open (§1.3, K-17). Since D-611 it appears in all three: `z_definer_abrechnungsfreigabe` and `app.zeit_zur_abrechnung_freigeben` (`drizzle/0366`), the binding (`drizzle/0371`) and the route gate on `/portal/[mandant]/zeiten/freigabe`.
