...
| R-20 | **Discharged.** The agent architecture is `06-AGENTEN-FREIGABEN.md`, and no sibling cites it live as `07-AGENTEN-ARCHITEKTUR.md` any more: `03-AUTH-BERECHTIGUNGEN.md` and `04-SEITENKARTE.md` never name that path at all, and the two remaining occurrences are explicit withdrawal notes — `01-ORDNERSTRUKTUR.md` §0 lists it among the names it says are "none of which is a real path", and `05-API-KARTE.md` §I records the correction | `01-ORDNERSTRUKTUR.md`, `05-API-KARTE.md` |
