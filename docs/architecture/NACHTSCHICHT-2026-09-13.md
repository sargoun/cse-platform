...
| 10:10 | **D-486**: PR 67 — Jahrespaket (ZIP je Wirtschaftsjahr: Z3-Tabellen, Monatszahlen, offene Posten, Ausgangsbuch, DATEV-Stapel mit Datei, Prüfbündel mit Belegen, Verfahrensdokumentation; jede Lücke im LIESMICH) und Lohnexport (Konten, Abwesenheiten, jeder Zeiteintrag aus dem MiLoG-Nachweis; Format hinter `LohnexportFormat` als Platzhalter, O-27/O-139). | Kern `lohnexport-format`; Isolation `jahrespaket-lohnexport` (4); Browser `jahrespaket-lohnexport.spec` |
| 11:40 | **D-487**: Nutzerbefund — Mitarbeiter-Anmeldung am Handy ohne SMS: Zugangsseite mit Anmeldecode aus der Hand der Einsatzleitung (0143 `app.zugang_code_ausstellen`, Keks statt Adresse, Protokoll ohne Code); Schichten: `/dienstplan/serien/neu` (Turnus + Serie + Schichten sofort, Posten-Serie, Absage für Bau), Posten mit Regel bekommt seine Serie beim Anlegen, Generator als Anwendungsrolle über `app.planungsbedarf_eigen`; Abwesenheitsart für den Lohnexport über `app.lohnexport_abwesenheiten`. | Kern `serie-regel`; Isolation `serie-zugangscode` (6); Browser `schichten-zugang.spec` (5, davon zwei auf 390 px) |
| 13:30 | **D-488**: derselbe Befund, zu Ende verfolgt — drei Ursachen hinter einem Bild: das Codefeld blockierte eingefügte Codes stumm (`pattern`), „richtiger Code, kein Konto" (0115) las sich wie „falscher Code", und ein `__Host-`-Keks über `http://` wird vom Browser verworfen. Neu: `nurZiffern`, zwei unterscheidbare Fehlerkästen, der Satz zum abgelehnten Keks, und `app.zugang_stand` (0144) — die Einsatzleitung sieht Zugang, Konto, offene Codes und letzte Anmeldung. | Kern `mitarbeiter-anmeldung` (+3); Isolation `serie-zugangscode` (9); Browser `schichten-zugang.spec` (7) |
