...
| O-889 | **Soll eine Benachrichtigung in der Sprache der Empfängerin entstehen (`person.sprache`) oder in der Sprache der Gesellschaft, die sie versendet?** Die Oberfläche des Arbeiterportals steht in vier Sprachen; der Posteingang trägt dagegen **gespeicherten** Text, und der ist in dem Moment festgelegt, in dem er entsteht — eine Meldung, deren Sprache sich später ändert, gibt es nicht. Gebaut ist deshalb der Weg, der am wenigsten erfindet: **die Meldung entsteht deutsch**, die Seite daneben übersetzt (`EINWAND_STATUS_TEXTE`, vier Sprachen), und die **Begründung der Planung bleibt unübersetzt** — sie ist die Aussage eines Menschen über einen Einzelfall, keine Beschriftung. Sagt der Auftraggeber, die Meldung müsse in der Sprache der Empfängerin stehen, ist die Änderung ein Parameter: `erzeuge()` bekommt die Sprache und die Artdefinition eine Tabelle statt einer Zeichenkette. | NOT-01, NOT-02, SPEC §10, D-419, `src/server/services/zeit/benachrichtigung.ts` |
