...
--     Kontoseite liest seit V-165 dieselbe Menge ueber diese Funktion, weil
--     sie daraus auch die Kopfzeile baut.
--
-- Die Policies unten fragen switcher_mandanten() in einer Unterabfrage:
-- so wertet der Planer sie EINMAL je Anweisung aus (InitPlan), nicht je
-- Zeile — dieselbe Form, die 01-KERN §1.3 fuer Policies vorschreibt (K-03).
-- Die Typangabe ::uuid[] dahinter ist noetig: ohne sie liest Postgres
-- = any (select ...) als Vergleich mit jeder ZEILE der Unterabfrage, und die
-- eine Zeile ist ein Feld — uuid gegen uuid[] bricht die Migration ab.
--
...
create policy d_umschalter_mandant on mandant
  ...
  using (id = any ((select app.switcher_mandanten())::uuid[]));
...
create policy d_umschalter_mitgliedschaft on benutzer_mandant
  for select to cse_definer
  using (benutzer_id = (select app.aktueller_benutzer()));
...
create policy d_umschalter_auftrag on auftrag
  for select to cse_definer
  using (mandant_id = any ((select app.switcher_mandanten())::uuid[]));
