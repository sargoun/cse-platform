...
export async function POST(anfrage: NextRequest): Promise<NextResponse> {
  ...
  /*
   * Der Rueckweg ist das Benutzerblatt, von dem das Formular kam — geprueft
   * auf den eigenen Ursprung (`internesZiel`). Faellt er durch, geht es auf
   * die Benutzerliste DIESER Gesellschaft; der Slug wird in derselben
   * Transaktion gelesen, damit auch der Fehlerzweig ihn hat.
   */
  let liste = '/portal';
  const rueckweg = (stand: string): NextResponse => {
    const ziel = internesZiel(daten.get('zurueck') as string | null, liste, anfrage);
  ...
  try {
    const { geaendert } = await (db().begin(async (tx: postgres.TransactionSql) =>
      withTenant(tx, sitzung, async (kontext) => {
        ...
        if (bereich !== undefined) liste = `/portal/${bereich.slug}/einstellungen/benutzer`;
