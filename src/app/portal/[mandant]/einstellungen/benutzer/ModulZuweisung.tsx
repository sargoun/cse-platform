...
export function ModulZuweisung({
  mandant, benutzerId, selbst, mitgliedschaften, katalog, sprache, t,
}: {
  readonly mandant: string;
  readonly benutzerId: string;
  readonly selbst: boolean;
  readonly mitgliedschaften: readonly ZuweisbareMitgliedschaft[];
  readonly katalog: readonly string[];
  readonly sprache: InternSprache;
  readonly t: ModulZuweisungTexte;
}) {
  if (mitgliedschaften.length === 0) return null;
  ...
  return (
    <section className="mb-s6" data-cse="modul-zuweisung">
      ...
      {selbst ? (
        ...
        return (
          <form key={m.id} method="post" action="/api/einstellungen/mitgliedschaft-module"
            ...
            <fieldset className="border-0 p-0">
              ...
              <ul className="m-0 grid list-none grid-cols-1 gap-x-s4 p-0 sm:grid-cols-2 lg:grid-cols-3">
                {katalog.map((modul) => (
                  <li key={modul}>
                    <label title={modul}
                           className="flex min-h-11 cursor-pointer items-center gap-s2 text-sm text-text">
            ...
            <div>
              <Button type="submit" variante="primary" data-cse="modul-speichern">
                {t.speichern}
              </Button>
            </div>
          </form>
