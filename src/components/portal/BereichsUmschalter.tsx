...
export interface BereichsUmschalterProps {
   ...
   * Seitentitel; am Telefon trägt der Auslöser dann nur Zeichen und Chevron,
   * der Name erscheint ab `sm` (DESIGN §8: keine Zeile über den Rand).
   */
  ...
  return (
    <div className="relative">
      <button
        ...
        aria-label={`${texte.ausloeser}: ${gruppenansicht ? texte.gruppenuebersicht
          : aktiverBereich?.name ?? texte.bereichWaehlen}`}
        className="flex h-11 min-w-11 shrink-0 items-center gap-s2 rounded-md px-s2
                   hover:bg-surface-2"
      ...
      >
        ...
        <span className={`text-sm font-semibold text-text ${knapp ? 'hidden sm:inline' : ''}`}>
          {gruppenansicht ? texte.gruppenuebersicht : aktiverBereich?.name ?? texte.bereichWaehlen}
        </span>
